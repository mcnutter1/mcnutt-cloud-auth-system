...
if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  if($choice==='everywhere' && $actorType && $actorId){
    $auth->revokeAllForPrincipal($actorType,$actorId);
    log_event($pdo,$actorType,$actorId,'logout',['everywhere'=>true,'via'=>'confirm']);
  } elseif($token && $row){
    $pdo->prepare("UPDATE sessions SET revoked_at=NOW() WHERE id=?")->execute([$row['id']]);
    log_event($pdo,$row['user_type'],(int)$row['user_id'],'logout',['everywhere'=>false,'via'=>'confirm']);
  }
